...
class SimulationRunner(QRunnable):
    ...
    def include_init_options(self):
        ...
        if init_options.sources_sinks:
            if init_options.sources_sinks.lizard_raster_sources_sinks:
                ...
                self.tc.create_lizard_raster_sources_sinks(
                    sim_id, **lizard_raster_sources_sinks_data
                )
            if init_options.sources_sinks.lizard_timeseries_sources_sinks:
                ...
                self.tc.create_lizard_timeseries_sources_sinks(
                    sim_id, **lizard_timeseries_sources_sinks_data
                )
            if init_options.sources_sinks.timeseries_sources_sinks:
                ...
                self.tc.create_timeseries_sources_sinks(
                    sim_id, **timeseries_sources_sinks_data
                )
        ...
        if init_options.local_timeseries_rain:
            if init_options.local_timeseries_rain.lizard_timeseries_rain:
                ...
                self.tc.create_lizard_timeseries_rain(
                    sim_id, **lizard_timeseries_rain_data
                )
    ...
    def include_substances(self):
        ...
        if self.current_simulation.substances:
            ...
            for substance in substances:
                substance_from_api = self.tc.create_simulation_substances(
                    sim_id, **substance
                )
    ...
    def include_boundary_conditions(self):
        ...
        def upload_file_boundary_conditions(filename, filepath):
            bc_upload = self.tc.create_simulation_boundarycondition_file(
                sim_id, filename=filename
            )
    ...
    def include_structure_controls(self):
        ...
        def upload_file_structure_controls(filename, filepath, offset):
            sc_upload = self.tc.create_simulation_structure_control_file(
                sim_id, filename=filename, offset=offset
            )
            upload_local_file(sc_upload, filepath)
            for ti in range(int(self.upload_timeout // 2)):
                uploaded_files = {
                    scf.file.filename: scf
                    for scf in self.tc.fetch_structure_control_files(sim_id)
                }
        ...
        if structure_controls.file_structure_controls:
            ...
            sc_file_download = self.tc.fetch_structure_control_file_download(
                sim_temp_id, sc_file.id
            )
        ...
        if structure_controls.memory_structure_controls:
            sc_memory_data = {
                k: v
                for k, v in structure_controls.memory_structure_controls.to_dict().items()
        ...
        if structure_controls.table_structure_controls:
            sc_table_data = {
                k: v
                for k, v in structure_controls.table_structure_controls.to_dict().items()
                if k not in ignore_keys
            }
        ...
        if structure_controls.timed_structure_controls:
            sc_timed_data = {
                k: v
                for k, v in structure_controls.timed_structure_controls.to_dict().items()
                if k not in ignore_keys
            }
        ...
        if initial_conditions.initial_waterlevels_1d is not None:
            ...
            for ti in range(int(self.upload_timeout // 2)):
                ...
                if uploaded_initial_waterlevel.state == ThreediFileState.VALID.value:
                    ...
                    uploaded_initial_waterlevel.state == ThreediFileState.INVALID.value
                ):
                    state_detail = (
                        str(uploaded_initial_waterlevel.state_detail)
                        .strip("{}")
                        .strip()
                    )
            ...
            if uploaded_initial_waterlevel.state != ThreediFileState.VALID.value:
                state_detail = (
                    str(uploaded_initial_waterlevel.state_detail).strip("{}").strip()
                )
                err_msg = f"Failed to upload Initial Waterlevel file due to the following reasons: {state_detail}"
                raise SimulationRunnerError(err_msg)
    ...
    def include_laterals(self):
        ...
        if self.current_simulation.laterals:
            ...
            # Add File laterals
            file_lateral_1d_values = list(
                self.current_simulation.laterals.file_laterals_1d.values()
            )
            file_lateral_2d_values = list(
                self.current_simulation.laterals.file_laterals_2d.values()
            )
    ...
    def include_breaches(self):
        ...
        if self.current_simulation.breaches:
            for potential_breach in (
                self.current_simulation.breaches.potential_breaches or []
            ):
                breach_obj = self.tc.fetch_3di_model_point_potential_breach(
                    threedimodel_id, potential_breach.breach_id
                )
    ...
    def include_precipitation(self):
        ...
        if self.current_simulation.precipitation:
            precipitation_type = (
                self.current_simulation.precipitation.precipitation_type
            )
            ...
            for substance in substances:
                substance_name = substance.get("substance")
                substance_id = self.substances[
                    substance_name
                ]  # this is the substance ID returned by API
            ...
            if precipitation_type == RainEventTypes.CONSTANT.value:
                ...
                for values_chunk in self.split_to_even_chunks(values, 300):
                    ...
                    for substance in substances:
                        substance_value = substance["concentrations"][0][1]
                        substance["concentrations"] = [
                            [t - chunk_offset, substance_value] for t, _ in values_chunk
                        ]
                ...
                if netcdf_global:
                    upload = self.tc.create_simulation_global_netcdf_precipitation(
                        sim_id, filename=filename
                    )
                else:
                    upload = self.tc.create_simulation_raster_netcdf_precipitation(
                        sim_id, filename=filename
                    )
                ...
                for substance in substances:
                    substance_value = substance["concentrations"][0][1]
                    substance["concentrations"] = [
                        [t, substance_value] for t, _ in values
                    ]
    ...
    def include_wind(self):
        ...
        if self.current_simulation.wind:
            ...
            self.tc.create_simulation_initial_wind_drag_coefficient(
                sim_id, value=drag_coefficient
            )
    ...
    def include_lizard_post_processing(self):
        ...
        if lizard_post_processing:
            ...
            if lizard_post_processing.arrival_time_map:
                self.tc.create_simulation_postprocessing_in_lizard_arrival(
                    sim_id, basic_post_processing=True
                )
        ...
        if new_saved_state:
            if new_saved_state.thresholds:
                self.tc.create_simulation_saved_state_stable_threshold(
                    ...
                    thresholds=new_saved_state.thresholds,
                )
            else:
                self.tc.create_simulation_saved_state_timed(
                    ...
                    name=new_saved_state.name,
                    tags=new_saved_state.tags,
                    time=new_saved_state.time
                    if new_saved_state.time >= 0
                    else duration,
                )
