...
class WaterQualityCustomizedResultArea(BaseModel):
    """
    ...
    subsets: Annotated[List[StrictStr], Field(max_length=4)] = Field(
        description="Subsets of the customized results area. Valid subsets are: 1D, 2D, 1D2D, GW."
    )
    geometry: StrictStr = Field(
        description="Geometry of the customized results area, WKT format, must be WGS84."
    )
    __properties: ClassVar[List[str]] = ["url", "name", "subsets", "geometry"]

    @field_validator("subsets")
    def subsets_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(["1D", "2D", "1D2D", "GW"]):
                raise ValueError(
                    "each list item must be one of ('1D', '2D', '1D2D', 'GW')"
                )
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WaterQualityCustomizedResultArea from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WaterQualityCustomizedResultArea from a dict"""
        if obj is None:
            return None
